...
class AboutStrings < Neo::Koan
  ...
  def test_use_flexible_quoting_to_handle_really_hard_cases
    ...
    # The flexible quote operator allows us to define any long string by letting we choose (hence flexible!)
...
EOS
  ...
  def test_the_shovel_operator_modifies_the_original_string
    ...
    # That's cause << alters the original string rather than creating a new one, so it has better improvements in regards to memory?
  end

  def test_double_quoted_string_interpret_escape_characters
    string = "\n"
    assert_equal 1, string.size
    # Technically the "\n" is only one character, because is the interpreted as the newline character.
  end
  ...
  def test_any_ruby_expression_may_be_interpolated
    ...
    #  ANY function in ruby may be interpolated into a string
  end
